...
class Douban(object):
    ...
    async def get_fav_list(self):
        selector = await util.get_selector(url=self.fav_url, headers=self.headers)
    ...
    async def get_fav_item(self, comment=''):
        url = self.url
        self.aurl = url
        douban = OrderedDict()
        douban['aurl'] = url
        ...
        try:
            if url.find('note') != -1:
                await self.get_douban_note(url)
            elif url.find('book.douban.com/review') != -1:
                await self.get_douban_book_review(url)
            elif url.find('movie.douban.com/review') != -1:
                await self.get_douban_movie_review(url)
            elif url.find('status') != -1:
                await self.get_douban_status(url)
            elif url.find('group/topic') != -1:
                await self.get_douban_group_article(url)
        except Exception:
    ...
    async def get_douban_note(self, url):
        if self.scraper == 'Selenium':
            selector = util.get_page_by_selenium(url, headers=self.headers)
        else:
            selector = await util.get_selector(url, headers=self.headers)
    ...
    async def get_douban_book_review(self, url):
        selector = await util.get_selector(url, headers=self.headers)
    ...
    async def get_douban_movie_review(self, url):
        selector = await util.get_selector(url, headers=self.headers)
        self.title = selector.xpath('string(//div[@id="content"]//h1//span)')
        self.content = str(
    ...
    async def get_douban_status(self, url):
        selector = await util.get_selector(url, headers=self.headers)
    ...
    async def get_douban_group_article(self, url):
        selector = await util.get_selector(url, headers=self.headers)
        self.title = selector.xpath('string(//div[@id="content"]//h1)')
